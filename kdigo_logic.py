...
def check_renoprotection(data):
    ...
    if data['uacr'] > 300 or (data['has_htn'] and data['uacr'] > 30):
        if not data['on_ace_arb']:
            recommendations.append("⚠️ **MISSING:** ACEi or ARB indicated for Albuminuria/HTN.")

    # 2. SGLT2i Check (The "Flozin" Rule)
    if data['egfr'] >= 20 and (data['has_dm'] or data['has_ckd']):
        if not data['on_sglt2']:
            recommendations.append("⚠️ **MISSING:** SGLT2 Inhibitor indicated (eGFR > 20).")

    # 3. BP Check
    if data['sbp'] > 120:
        recommendations.append(f"⚠️ **BP ALERT:** SBP {data['sbp']} > 120 mmHg (KDIGO Target).")
