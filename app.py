...
if mode == "Clinic (Chronic)":
    st.header("🛡️ Renoprotection & Progression")
    ...
    with r2:
        ...
        if recs:
            ...
            st.success("✅ Excellent Guideline Adherence!")
...
# --- ER MODE LOGIC ---
    ...
    st.header("🚨 Acute Threat Assessment")
    st.error("Checking Life Threats (AEIOU)...")
    
    if potassium > 6.0:
        st.error(f"🛑 CRITICAL: Hyperkalemia (K+ {potassium}). Order ECG & Shift K+.")
